...
function _load_querystring() {
  return _querystring = require('querystring');
}
...
function _load_lodash() {
  return _lodash = _interopRequireDefault(require('lodash.omit'));
}
...
const DEFAULT = {
  ...
  constructor(settings = {}) {
    ...
    Object.keys((0, (_lodash || _load_lodash()).default)(DEFAULT, 'clientSecret', 'keychain')).forEach(key => {
     ...
     * @type {config}
     */
    this.config = (0, (_lodash || _load_lodash()).default)(config, 'keychain');
    /**
     * @type {Keychain}
     */
     ...
     * @type {EventEmitter}
     */
    this.events = new (_events || _load_events()).EventEmitter();
  ...
  getAccessToken(data, options = {}) {
    ...
    options = _extends({}, options, {
      ...
      method: 'POST',
      body: (0, (_querystring || _load_querystring()).stringify)(data)
    });

    return fetch(`${this.config.baseUrl}${this.config.grantPath}`, options).then(response => {
  ...
  async revokeToken(data, options = {}) {
    try {
      ...
      return fetch(`${this.config.baseUrl}${this.config.revokePath}`, options).then(T => T.json()).then(this.keychain.removeToken).catch(this.keychain.removeToken);
    } catch (err) {
      return Promise.reject(err);
    }
  }
   ...
   */
  isAuthenticated() {
    return this.keychain.getToken().then(token => Boolean(typeof token === 'object' && token['access_token']));
  }
}
